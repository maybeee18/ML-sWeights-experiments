...
# https://github.com/SiLiKhon/my_roc_auc/blob/master/my_roc_auc.py
def weighted_roc_auc(classes : np.ndarray,
               predictions : np.ndarray,
               weights : np.ndarray = None) -> float:
    """
    Calculating ROC AUC score as the probability of correct ordering for weighted points by @SiLiKhon.
    Permits 
    """
...
def bootstrap_score(labels, p, scorer, weights=None):
    scores = []
    for i in range(10):
        selection = np.random.choice(len(labels), replace=True, size=len(labels))
        if weights is not None:
            scores.append(scorer(labels[selection], p[selection], weights[selection]))
        else:
            scores.append(scorer(labels[selection], p[selection]))
